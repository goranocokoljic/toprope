# Toprope Documentation

**The complete user manual for Toprope — the AI adoption intelligence platform for engineering teams.**

This manual covers everything from installing Toprope to connecting your tools,
...
Read top to bottom the first time you deploy Toprope; afterwards use it as a
reference. Each chapter is self-contained.

### Getting started
1. [Introduction](./introduction.md) — what Toprope is, who it's for, and the problem it solves
...
4. [Configuration](./configuration.md) — the complete `toprope.config.yaml` reference
...
Toprope collects data from four kinds of source, normalizes it into a unified
...
Every insight is **tier-aware**: Toprope is always honest about how much it
...
- `npx toprope <cmd>` runs the compiled CLI (requires `npm run build` first).
