...
/**
 * Build the full detail payload for a developer, or null if the id is unknown.
 * Shared by the admin route (/api/developers/:id) and the session-scoped
 * developer route (/api/me/profile) so both return an identical shape and the
 * scoping decision lives entirely at the route layer.
 */
export function getDeveloperDetail(db: Database.Database, id: string): DeveloperDetail | null {
    const dev = db
