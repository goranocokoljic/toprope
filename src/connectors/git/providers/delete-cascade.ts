...
export interface ProviderDeleteImpact {
    ...
    /** Stored `git_last_sync` / `git_earliest_sync` / `git_stall` keys that would be purged. */
    cursor_keys: number;
    /**
...
/** How many of a container's cursor keys are actually stored right now. */
function countStoredCursorKeys(
    db: Database.Database,
    type: GitProviderType,
    container: string,
): number {
    const keys = containerCursorKeys(type, container);
    const placeholders = keys.map(() => '?').join(', ');
    return (
        db
            .prepare(`SELECT COUNT(*) AS n FROM sync_state WHERE key IN (${placeholders})`)
...
export function providerDeleteImpact(
    ...
    const cascadeSkipped = configContainerKeys.has(containerKey(type, container));
    ...
    return {
        provider: type,
        container,
        // On the skipped path nothing is removed, so every count is honestly 0 rather than
        // "what would have gone" — the confirmation must not describe a retraction that the
        // config sibling prevents.
        raw_author_rows: cascadeSkipped ? 0 : raw.rows,
        days: cascadeSkipped ? 0 : raw.days,
        earliest_date: cascadeSkipped ? null : raw.earliestDate,
        latest_date: cascadeSkipped ? null : raw.latestDate,
        commits: cascadeSkipped ? 0 : raw.commits,
        pr_records: cascadeSkipped ? 0 : prRecords,
        authors: cascadeSkipped ? 0 : raw.authors,
        developers_affected: cascadeSkipped ? 0 : developersAffectedByContainer(db, type, container),
        cursor_keys: cascadeSkipped ? 0 : countStoredCursorKeys(db, type, container),
...
export function deleteProviderWithCascade(
    ...
    return db.transaction((): ProviderDeleteResult => {
        // Existence and lifecycle state are read INSIDE the transaction, so the cascade acts
        // on the row it verified rather than on a snapshot taken before the write lock.
        ...
        const empty: ProviderDeleteResult = {
            id,
            provider: type,
            container,
            raw_author_rows: 0,
