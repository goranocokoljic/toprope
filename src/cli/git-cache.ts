...
export function clearDiffstatCache(
    ...
    const before = countDiffstats(db, scope);
    const removed = deleteDiffstats(db, scope);
    ...
    // State the CONSEQUENCE, not just the row count. The rows are gone permanently and the
    // re-fetch they cost is the expensive phase of a sync — thousands of API calls on a wide
    // purge — so an operator who mis-scoped this should learn it here rather than from an
    // unexplained multi-hour run. `before.absent` is called out because a purge aimed at a
    // frozen permission-revocation 404 (residual #1 in migration 044) is aimed exactly at
    // those rows.
    return {
        ok: true,
        removed,
        message:
            ...
            `(${before.absent} of them the "no diffstat exists" marker). No data was lost and ` +
            'no metric changed — these rows are a memo of an immutable remote fact. The next ' +
            'sync that reaches these commits will re-fetch them, one API call each.',
    };
}
